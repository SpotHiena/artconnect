...
<body>
  ...
  <div class="container form-container">
    <a href="galeria.php?id=<?= $_SESSION['ID'] ?>" class="btn btn-outline-secondary btn-sm btn-voltar">← Voltar</a>
    ...
    <form action="acoes.php" method="post" enctype="multipart/form-data">
      ...
      <!-- Nova opção de venda -->
      <div class="form-label mt-3">Deseja vender a arte?</div>
      <div class="radio-venda">
        <div class="form-check form-check-inline">
          <input class="form-check-input" type="radio" name="venda" id="venda_sim" value="1">
          <label class="form-check-label" for="venda_sim">Sim, vender</label>
        </div>
        <div class="form-check form-check-inline">
          <input class="form-check-input" type="radio" name="venda" id="venda_nao" value="0" checked>
          <label class="form-check-label" for="venda_nao">Não, só postar</label>
        </div>
      </div>

      <div id="venda-container" style="display:none;">
        <!-- Promoção -->
        ...
        <div class="precos-linha mt-2">
          <div>
            <label for="preco" class="form-label">Preço de Venda:</label>
            <input type="text" name="preco" id="preco" class="form-control" data-mask="000000,00"
              data-mask-reverse="true">
          </div>
          <div>
            <label for="desconto" class="form-label">% Desconto:</label>
            <div class="input-group">
              <input type="number" name="desconto" id="desconto" class="form-control" min="0" max="100" step="1">
  ...
  <script>
    ...
    // Controle de exibição dos campos de venda e promoção
    const vendaSim = document.getElementById('venda_sim');
    const vendaNao = document.getElementById('venda_nao');
    const vendaContainer = document.getElementById('venda-container');
    ...
    vendaSim.addEventListener('change', atualizarCamposVenda);
    vendaNao.addEventListener('change', atualizarCamposVenda);
    atualizarCamposVenda(); // Atualiza ao carregar a página
  </script>
